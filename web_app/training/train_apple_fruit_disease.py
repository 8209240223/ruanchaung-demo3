...
# 果实数据集路径（任选其一存在即可）
_dataset_candidates = [
    os.path.join(WEB_APP_DIR, "training", "fruit_disease_dataset", "dataset"),
    os.path.join(SCRIPT_DIR, "fruit_disease_dataset", "dataset"),
]
DEFAULT_DATA_DIR = next((p for p in _dataset_candidates if os.path.isdir(p)), _dataset_candidates[0])
DEFAULT_OUTPUT_DIR = os.path.join(WEB_APP_DIR, "training", "fruit_processed_dataset")
IMAGE_SIZE = 224
...
def _find_dataset_root():
    ...
    """
    search_roots = [
        os.path.join(WEB_APP_DIR, "training"),
        WEB_APP_DIR,
    ]
    checked = set()
